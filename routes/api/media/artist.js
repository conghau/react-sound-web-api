...
const rq = (type, name, page) => {
  let options = {
    uri: `http://mp3.zing.vn/nghe-si/${name}/${type}`,
    headers: {
  ...
  if (page) {
    return request(Object.assign({}, options, {uri: `http://mp3.zing.vn/nghe-si/${name}/${type}?&page=${page}`}));
  }
...
const getInformation = (name, res, next) => {
  rq('tieu-su', name)
    .then(html => {
      const result = {
        fullName: '',
