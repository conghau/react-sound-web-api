...
PageScraper.prototype.getPaginate = function () {
  const lastPage = this.$('.pagination ul li').last().children()[0];
  if (lastPage) {
    const lastPageHref = lastPage.attribs.href;
    return this.result.numberOfPages = parseInt(/\d+$/g.exec(lastPageHref)[0], 10);
  } else {
    return this.result.numberOfPages = 1;
  }
};
...
PageScraper.prototype.get = function () {
  ...
  this.elements.each((index, element) => {
    ...
    // for (let selector in this.attrs) {
    //   if (this.attrs.hasOwnProperty(selector)) {
    //     const arr = this.attrs[selector];
    //     const $el = this.$(element).find(selector);
    //     for (let key in arr) {
    //       if(arr.hasOwnProperty(key)) {
    //         const {attr, outputKey, manipulateFunc} = arr[key];
    //         if (outputKey === 'id' || outputKey === 'alias') {
    //           // get the song id and alias
    //           const match = /\/([0-9A-Za-z_-]+)\/(\w+)\.html/.exec(this.attr($el, attr));
    //           const [, alias, id] = match;
    ...
    //       const {attr, outputKey, manipulateFunc} = arr[j];
    //       if (outputKey === 'id' || outputKey === 'alias') {
    //         // get the song id and alias
    //         const match = /\/([0-9A-Za-z_-]+)\/(\w+)\.html/.exec(this.attr($el, attr));
    //         const [, alias, id] = match;
  ...
  console.timeEnd('PageScraper_gET');
  return this.result;
};
